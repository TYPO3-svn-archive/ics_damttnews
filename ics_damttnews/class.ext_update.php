 ...
 *
 * @author Virginie Sugere <virginie.sugere@in-cite.net>
 * @package TYPO3-DAM
 * @subpackage tx_dam
 */
class ext_update  {
			...
			foreach($entry as $type => $nEntry)
			{
				...
				foreach($nEntry as $uid_local)
				{
					$insert = array(
						...
						'ident' => ($type == 'pics' ? 'tx_icsdamttnews_images' : 'tx_icsdamttnews_media'),
						'tablenames' => 'tt_news',
						'sorting' => '0',
					...
					if($type == 'pics') {
						if(!$GLOBALS['TYPO3_DB']->exec_UPDATEquery('tt_news', 'uid=' . $uid_foreign,
							array('image' => '', 'tx_icsdamttnews_images' => count($entries[$uid_foreign][$type]))))
							++$error;
					}
					else {
						if(!$GLOBALS['TYPO3_DB']->exec_UPDATEquery('tt_news', 'uid=' . $uid_foreign,
							array('news_files' => '', 'tx_icsdamttnews_media' => count($entries[$uid_foreign][$type]))))
							++$error;
					}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/ics_damttnews/class.ext_update.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/ics_damttnews/class.ext_update.php']);
}
