...
class user_class
{
	function userTCAformDAM($PA, $fobj)
	{
  	...
  	$res = $GLOBALS['TYPO3_DB']->exec_SELECT_mm_query('tx_dam.description, tx_dam.alt_text, ' .
			...
			'AND tx_dam_mm_ref.tablenames="tt_news" AND tx_dam_mm_ref.ident="tx_icsdamttnews_images" ' .
		...
		return '
			...
			<div>
				<input type="button" name="c1" value="' . $this->tceforms->sL('LLL:EXT:ics_damttnews/locallang_db.xml:tt_news.copy1', true) . '" onclick="copyDAMfield(\'imagecaption\');return false;">
				<input type="button" name="c2" value="' . $this->tceforms->sL('LLL:EXT:ics_damttnews/locallang_db.xml:tt_news.copy2', true) . '" onclick="copyDAMfield(\'imagealttext\');return false;">
				<input type="button" name="c3" value="' . $this->tceforms->sL('LLL:EXT:ics_damttnews/locallang_db.xml:tt_news.copy3', true) . '" onclick="copyDAMfield(\'imagetitletext\');return false;">
			</div>';
	}
}
